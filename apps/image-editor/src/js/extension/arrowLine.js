...
class ArrowLine extends fabric.Line {
  ...
  clone() {
    const options = this.toObject(['arrowType']);

    return Promise.resolve(
      new ArrowLine([options.x1, options.y1, options.x2, options.y2], options)
    );
  }
}
...
Object.assign(ArrowLine.prototype, {
  /**
   * Render ArrowLine
   * @private
   * @override
  ...
  _drawChevronPath(type, { fromX, fromY, toX, toY }, decorateSize) {
    ...
    const angle =
      (Math.atan2(compareY - standardY, compareX - standardX) * RADIAN_CONVERSION_VALUE) / Math.PI;
